...
<body>
	...
	?>
	<main class=" manual contadores">
		<div class="col-xs-12 col-md-12 offset-md-1 m-0 p-0">
			<div class=" div_buscador">
				<div class="container position-relative">
					<div class="div_botones d-flex">
						<a class="btn_inicio contadores" href="/"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-home">
								<path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
								<polyline points="9 22 9 12 15 12 15 22"></polyline>
							</svg></a>
						<div class="separador"><span>|</span></div>
						<a class="btn_volver contadores" href="/"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-left">
			...
			<div class="container d-flex" style="white-space: nowrap; flex-wrap: wrap;">
				...
				require_once "Database.php";
				$db = Database::getInstance();
				$sql = "SELECT * FROM categorias WHERE plataforma = 'contadores' AND sub_categoria_id = 0 ORDER BY orden ASC";
				$stm = $db->query($sql);
				$categorias = $stm->fetchAll();
				foreach ($categorias as $row) {
				?>
					<span id="tag_<?= $row['id'] ?>" onclick="BuscarSubCategorias(<?= $row['id'] ?>, 2)" class="tags_topicos"><?= $row["topico"] ?></span>
				<?php
				}
				?>
			</div>
